package com.iodigital.lokex.utils
...
private val statusLength = 96
private var status = "Starting LokEx..."
...
fun status(message: String, finalStatus: Boolean = false) {
    if (!showStatus) return
...
private fun log(level: String, tag: String, message: String, throwable: Throwable? = null) =
    runBlocking {
        logMutex.withLock {
            ...
            message.split("\n").forEach {
                val line = "\r" + listOf(level, date, paddedTag, it).joinToString(" | ")
                System.out.flush()
